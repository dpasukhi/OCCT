...
TEST_F(ExtremaPS_PlaneTest, ProjectionOutsideOneBoundUsesEdgeInterior)
{
  const gp_Pln             aPlane(gp_Pnt(0.0, 0.0, 0.0), gp_Dir(0.0, 0.0, 1.0));
  const gp_Pnt             aPoint(5.0, 20.0, 3.0);
  ExtremaPS_Plane          anEval(aPlane, ExtremaPS::Domain2D(0.0, 10.0, 0.0, 10.0));
  ...
  EXPECT_NEAR(aMinimum.U, 5.0, THE_TOL);
  EXPECT_NEAR(aMinimum.V, 10.0, THE_TOL);
  EXPECT_NEAR(aMinimum.SquareDistance, 109.0, THE_TOL);
  EXPECT_TRUE(aMinimum.IsMinimum);
}

TEST_F(ExtremaPS_PlaneTest, PartiallyUnboundedDomainPreservesFiniteBoundary)
{
  const gp_Pln    aPlane(gp_Pnt(0.0, 0.0, 0.0), gp_Dir(0.0, 0.0, 1.0));
  const gp_Pnt    aPoint(-5.0, 2.0, 3.0);
  ExtremaPS_Plane anEval(
    aPlane,
    ExtremaPS::Domain2D(0.0, 10.0, -Precision::Infinite(), Precision::Infinite()));

  EXPECT_EQ(anEval.Perform(aPoint, THE_TOL, ExtremaPS::SearchMode::Min).Status,
            ExtremaPS::Status::NoSolution);
  ...
  ASSERT_NE(aResult.MinIndex(), ExtremaPS::Result::INVALID_INDEX);
  const ExtremaPS::ExtremumResult& aMinimum = aResult[aResult.MinIndex()];
  EXPECT_NEAR(aMinimum.U, 0.0, THE_TOL);
  EXPECT_NEAR(aMinimum.V, 2.0, THE_TOL);
  EXPECT_NEAR(aMinimum.SquareDistance, 34.0, THE_TOL);
}
