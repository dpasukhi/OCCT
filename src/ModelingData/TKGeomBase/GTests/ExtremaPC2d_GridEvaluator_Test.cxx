...
TEST(ExtremaPC2d_GridEvaluatorTest, UniformParamsCoverDomainExactly)
{
  ...
  ASSERT_EQ(aParams.Length(), 7);
  EXPECT_EQ(aParams(1), -3.0);
  EXPECT_EQ(aParams(7), 5.0);
  for (int anIndex = 2; anIndex <= 7; ++anIndex)
  {
    EXPECT_GT(aParams(anIndex), aParams(anIndex - 1));
  }
}

TEST(ExtremaPC2d_GridEvaluatorTest, UniformParamsRejectInvalidSampleCount)
{
  EXPECT_THROW(ExtremaPC2d_GridEvaluator::BuildUniformParams(0.0, 1.0, 1),
               Standard_RangeError);
}
...
TEST(ExtremaPC2d_GridEvaluatorTest, InvalidPartitionAndToleranceAreRejected)
{
  ...
  EXPECT_EQ(anEvaluator.Perform(anAdaptor, gp_Pnt2d(), {0.0, 1.0}, 1.0e-8,
                                 ExtremaPC2d::SearchMode::MinMax).Status,
            ExtremaPC2d::Status::InvalidInput);

  math_Vector aParams(1, 3);
  aParams(1) = 0.0;
  aParams(2) = 0.5;
  aParams(3) = 1.0;
  anEvaluator.SetParams(aParams);
  EXPECT_EQ(anEvaluator.Perform(anAdaptor, gp_Pnt2d(), {1.0, 0.0}, 1.0e-8,
                                 ExtremaPC2d::SearchMode::MinMax).Status,
            ExtremaPC2d::Status::InvalidInput);
  EXPECT_EQ(anEvaluator.Perform(anAdaptor, gp_Pnt2d(), {0.0, 1.0}, 0.0,
                                 ExtremaPC2d::SearchMode::MinMax).Status,
            ExtremaPC2d::Status::InvalidInput);
}
