...
TEST_F(ExtremaPS_SurfaceTest, NullSurfaceHasLocalNotDoneResult)
{
  ExtremaPS_Surface anEvaluator{occ::handle<Geom_Surface>()};

  EXPECT_FALSE(anEvaluator.IsInitialized());
  EXPECT_EQ(anEvaluator.Perform(gp_Pnt(1.0, 2.0, 3.0), THE_TOL).Status, ExtremaPS::Status::NotDone);
  EXPECT_EQ(anEvaluator.PerformWithBoundary(gp_Pnt(1.0, 2.0, 3.0), THE_TOL).Status,
            ExtremaPS::Status::NotDone);
}
...
TEST_F(ExtremaPS_SurfaceTest, Result_Empty)
{
  ...
  EXPECT_EQ(aResult.MinIndex(), ExtremaPS::Result::INVALID_INDEX);
...
TEST_F(ExtremaPS_SurfaceTest, TransformedAdaptorUsesTransformedGeometry)
{
  const occ::handle<Geom_Plane> aPlane =
    new Geom_Plane(gp_Pln(gp_Pnt(0.0, 0.0, 0.0), gp_Dir(0.0, 0.0, 1.0)));
  gp_Trsf aTranslation;
  aTranslation.SetTranslation(gp_Vec(10.0, -4.0, 7.0));

  const GeomAdaptor_TransformedSurface anAdaptor(aPlane, aTranslation);
  const ExtremaPS::Domain2D            aDomain(-20.0, 20.0, -20.0, 20.0);
  const ExtremaPS_Surface              anExtrema(anAdaptor, aDomain);
  const ExtremaPS::Result& aResult = anExtrema.Perform(gp_Pnt(12.0, -1.0, 12.0), THE_TOL);

  ASSERT_TRUE(aResult.IsDone());
  ASSERT_EQ(aResult.NbExt(), 1u);
  EXPECT_NEAR(aResult[0].Point.X(), 12.0, THE_TOL);
  EXPECT_NEAR(aResult[0].Point.Y(), -1.0, THE_TOL);
  EXPECT_NEAR(aResult[0].Point.Z(), 7.0, THE_TOL);
