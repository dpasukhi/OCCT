...
TEST_F(ExtremaPS_SurfaceOfRevolutionTest, OnAxisInteriorQueryDoesNotUseVEndpoints)
{
  ExtremaPS_SurfaceOfRevolution anExtPS(myCylinderLike,
                                        ExtremaPS::Domain2D(0.0, 2.0 * M_PI, 0.0, 20.0));

  const ExtremaPS::Result& aResult =
    anExtPS.Perform(gp_Pnt(0.0, 0.0, 30.0), THE_TOLERANCE, ExtremaPS::SearchMode::Min);

  EXPECT_EQ(aResult.Status, ExtremaPS::Status::NoSolution);
}

TEST_F(ExtremaPS_SurfaceOfRevolutionTest, OnAxisMaximumUsesFarthestMeridianRing)
{
  ExtremaPS_SurfaceOfRevolution anExtPS(myTorusLike,
                                        ExtremaPS::Domain2D(0.0, 2.0 * M_PI, 0.0, 2.0 * M_PI));

  const ExtremaPS::Result& aResult =
    anExtPS.Perform(gp_Pnt(0.0, 0.0, 0.0), THE_TOLERANCE, ExtremaPS::SearchMode::Max);

  ASSERT_EQ(aResult.Status, ExtremaPS::Status::InfiniteSolutions);
  EXPECT_NEAR(std::sqrt(aResult.InfiniteSquareDistance), 7.0, THE_DIST_TOLERANCE);
}

TEST_F(ExtremaPS_SurfaceOfRevolutionTest, NegativeMeridianRadiusUsesOppositeAngle)
{
  const occ::handle<Geom_Line> aLine =
    new Geom_Line(gp_Lin(gp_Pnt(0.0, 0.0, 0.0), gp_Dir(1.0, 0.0, 0.0)));
  const occ::handle<Geom_TrimmedCurve> aNegativeMeridian = new Geom_TrimmedCurve(aLine, -5.0, -1.0);
  const occ::handle<Geom_SurfaceOfRevolution> aSurface =
    new Geom_SurfaceOfRevolution(aNegativeMeridian,
                                 gp_Ax1(gp_Pnt(0.0, 0.0, 0.0), gp_Dir(0.0, 0.0, 1.0)));
  ExtremaPS_SurfaceOfRevolution anExtPS(aSurface, ExtremaPS::Domain2D(0.0, 2.0 * M_PI, -5.0, -1.0));

  const ExtremaPS::Result& aResult =
    anExtPS.Perform(gp_Pnt(3.0, 0.0, 2.0), THE_TOLERANCE, ExtremaPS::SearchMode::Min);

  ASSERT_TRUE(aResult.IsDone());
  ASSERT_NE(aResult.MinIndex(), ExtremaPS::Result::INVALID_INDEX);
  const ExtremaPS::ExtremumResult& aMinimum = aResult[aResult.MinIndex()];
  EXPECT_NEAR(aMinimum.U, M_PI, THE_TOLERANCE);
  EXPECT_NEAR(aMinimum.V, -3.0, THE_TOLERANCE);
  EXPECT_NEAR(aMinimum.Point.Distance(gp_Pnt(3.0, 0.0, 0.0)), 0.0, THE_TOLERANCE);
  EXPECT_NEAR(aMinimum.SquareDistance, 4.0, THE_TOLERANCE);
}

TEST_F(ExtremaPS_SurfaceOfRevolutionTest, MeridianPlaneIsDerivedFromGeometry)
{
  const occ::handle<Geom_Line> aMeridian = new Geom_Line(gp_Lin(gp_Pnt(0.0, 5.0, 0.0), gp::DZ()));
  const occ::handle<Geom_SurfaceOfRevolution> aSurface =
    new Geom_SurfaceOfRevolution(aMeridian, gp_Ax1(gp_Pnt(0.0, 0.0, 0.0), gp::DZ()));
  ExtremaPS_SurfaceOfRevolution anExtPS(aSurface);

  const gp_Pnt             aPoint(10.0, 0.0, 3.0);
  const ExtremaPS::Result& aResult =
    anExtPS.Perform(aPoint, THE_TOLERANCE, ExtremaPS::SearchMode::Min);

  ASSERT_EQ(aResult.Status, ExtremaPS::Status::OK);
  ASSERT_NE(aResult.MinIndex(), ExtremaPS::Result::INVALID_INDEX);
  const ExtremaPS::ExtremumResult& aMinimum = aResult[aResult.MinIndex()];
  EXPECT_NEAR(aMinimum.Point.Distance(gp_Pnt(5.0, 0.0, 3.0)), 0.0, THE_TOLERANCE);
  EXPECT_NEAR(aMinimum.SquareDistance, 25.0, THE_TOLERANCE);

  const Geom_Surface::ResD1 aD1 = aSurface->EvalD1(aMinimum.U, aMinimum.V);
  const gp_Vec              aResidual(aPoint, aD1.Point);
  EXPECT_NEAR(aResidual.Dot(aD1.D1U), 0.0, 1.0e-10);
  EXPECT_NEAR(aResidual.Dot(aD1.D1V), 0.0, 1.0e-10);
}
