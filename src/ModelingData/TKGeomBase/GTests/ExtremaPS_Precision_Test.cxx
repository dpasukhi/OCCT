...
protected:
                       ...
                       const char*                 theTestName)
  {
    ...
    const ExtremaPS::Result& aResult =
      anEval.PerformWithBoundary(thePoint, theTol, ExtremaPS::SearchMode::Min);
    ...
    for (size_t i = 0; i < aResult.NbExt(); ++i)
    {
      ...
      double aRelError =
        std::abs(aStoredSqDist - aComputedSqDist) / std::max(aComputedSqDist, 1.0e-20);
      EXPECT_LT(aRelError, theTol * 100.0) << theTestName << ": Distance mismatch at extremum " << i
                                           << ", relative error: " << aRelError;
...
TEST_F(ExtremaPS_PrecisionTest, StabilityAcrossMultipleRuns)
{
  ...
  for (int i = 0; i < 5; ++i)
  {
    ExtremaPS_Surface        anEval(anAdaptor, ExtremaPS::Domain2D(0.0, 1.0, 0.0, 1.0));
    const ExtremaPS::Result& aResult =
      anEval.Perform(aPoint, THE_HIGH_PRECISION_TOL, ExtremaPS::SearchMode::Min);
