...
TEST_F(ExtremaPS_TorusTest, AxisMaximumUsesFarthestRing)
{
  const gp_Pnt    aPoint(0.0, 0.0, 3.0);
  ExtremaPS_Torus anEval(myTorus, ExtremaPS::Domain2D(0.0, 2.0 * THE_PI, 0.0, 2.0 * THE_PI));

  const ExtremaPS::Result& aResult =
    anEval.Perform(aPoint, THE_TOLERANCE, ExtremaPS::SearchMode::Max);

  ASSERT_EQ(aResult.Status, ExtremaPS::Status::InfiniteSolutions);
  EXPECT_NEAR(std::sqrt(aResult.InfiniteSquareDistance), std::sqrt(34.0) + 1.0, THE_TOLERANCE);
}

TEST_F(ExtremaPS_TorusTest, AxisInteriorQueryHonorsVRange)
{
  const gp_Pnt    aPoint(0.0, 0.0, 0.0);
  ExtremaPS_Torus anEval(myTorus, ExtremaPS::Domain2D(0.0, 2.0 * THE_PI, 0.2, 1.0));

  const ExtremaPS::Result& aResult =
    anEval.Perform(aPoint, THE_TOLERANCE, ExtremaPS::SearchMode::MinMax);
...
TEST_F(ExtremaPS_TorusTest, SearchMode_MinMax)
{
  ...
  EXPECT_GE(aResult.Extrema.Size(), 2);
}
...
TEST_F(ExtremaPS_TorusTest, MultipleExtrema_MinMaxDifferent)
{
  ...
  ASSERT_EQ(aResult.Status, ExtremaPS::Status::OK);
  ASSERT_GE(aResult.Extrema.Size(), 2);
