...
TEST_F(ExtremaPS_ConeTest, PointOnAxis_Degenerate)
{
  ...
  EXPECT_TRUE(aResult.IsInfinite());
  EXPECT_NEAR(aResult.InfiniteSquareDistance, 50.0, THE_TOL);
}

TEST_F(ExtremaPS_ConeTest, InteriorQueryDoesNotClampGeneratrixParameter)
{
  const gp_Cone aCone(gp_Ax3(gp_Pnt(0, 0, 0), gp_Dir(0, 0, 1)), M_PI / 4.0, 0.0);
  const gp_Pnt  aPoint(0.0, 0.0, 40.0);
...
TEST_F(ExtremaPS_ConeTest, AxisHasNoInteriorMaximum)
{
  const gp_Cone aCone(gp_Ax3(gp_Pnt(0, 0, 0), gp_Dir(0, 0, 1)), M_PI / 4.0, 0.0);
  const gp_Pnt  aPoint(0.0, 0.0, 10.0);

  ExtremaPS_Cone           anEval(aCone, ExtremaPS::Domain2D(0.0, 2.0 * M_PI, 0.0, 20.0));
  const ExtremaPS::Result& aResult = anEval.Perform(aPoint, THE_TOL, ExtremaPS::SearchMode::Max);

  EXPECT_EQ(aResult.Status, ExtremaPS::Status::NoSolution);
}

TEST_F(ExtremaPS_ConeTest, AxisBoundaryMaximumUsesFarthestEndRing)
{
  const double  aSemiAngle = M_PI / 4.0;
  const gp_Cone aCone(gp_Ax3(gp_Pnt(0, 0, 0), gp_Dir(0, 0, 1)), aSemiAngle, 0.0);
  const gp_Pnt  aPoint(0.0, 0.0, 5.0);
  ...
  const double aRadius = 10.0 * std::sin(aSemiAngle);
  const double aDeltaZ = 5.0 - 10.0 * std::cos(aSemiAngle);
  ASSERT_EQ(aResult.Status, ExtremaPS::Status::InfiniteSolutions);
  EXPECT_NEAR(aResult.InfiniteSquareDistance, aRadius * aRadius + aDeltaZ * aDeltaZ, THE_TOL);
}
...
TEST_F(ExtremaPS_ConeTest, InteriorQueryDoesNotClampToVBoundary)
{
  const gp_Cone  aCone(gp_Ax3(gp_Pnt(0.0, 0.0, 0.0), gp_Dir(0.0, 0.0, 1.0)), M_PI / 4.0, 0.0);
  ExtremaPS_Cone anEval(aCone, ExtremaPS::Domain2D(0.0, 2.0 * M_PI, 0.0, 10.0));

  const ExtremaPS::Result& aResult =
    anEval.Perform(gp_Pnt(10.0, 0.0, 100.0), THE_TOL, ExtremaPS::SearchMode::Min);
