...
TEST_F(ExtremaPS_CylinderTest, InteriorQueryDoesNotClampAxialParameter)
{
  const gp_Cylinder aCylinder(gp_Ax3(gp_Pnt(0, 0, 0), gp_Dir(0, 0, 1)), 10.0);
  const gp_Pnt      aPoint(0.0, 0.0, 25.0);

  ExtremaPS_Cylinder       anEval(aCylinder, ExtremaPS::Domain2D(0.0, 2.0 * M_PI, 0.0, 20.0));
  const ExtremaPS::Result& aResult = anEval.Perform(aPoint, THE_TOL, ExtremaPS::SearchMode::Min);

  EXPECT_EQ(aResult.Status, ExtremaPS::Status::NoSolution);
}

TEST_F(ExtremaPS_CylinderTest, AxisBoundaryMaximumUsesFarthestEndRing)
{
  const gp_Cylinder aCylinder(gp_Ax3(gp_Pnt(0, 0, 0), gp_Dir(0, 0, 1)), 5.0);
  const gp_Pnt      aPoint(0.0, 0.0, 2.0);
  ...
  EXPECT_NEAR(aResult.InfiniteSquareDistance, 89.0, THE_TOL);
}
...
TEST_F(ExtremaPS_CylinderTest, InteriorQueryDoesNotClampToVBoundary)
{
  const gp_Cylinder  aCylinder(gp_Ax3(gp_Pnt(0.0, 0.0, 0.0), gp_Dir(0.0, 0.0, 1.0)), 10.0);
  ExtremaPS_Cylinder anEval(aCylinder, ExtremaPS::Domain2D(0.0, 2.0 * M_PI, 0.0, 20.0));

  const ExtremaPS::Result& aResult =
    anEval.Perform(gp_Pnt(10.0, 0.0, 25.0), THE_TOL, ExtremaPS::SearchMode::Min);

  EXPECT_TRUE(aResult.IsDone());
  EXPECT_EQ(aResult.Status, ExtremaPS::Status::NoSolution);
  EXPECT_EQ(aResult.NbExt(), 0U);
}

TEST_F(ExtremaPS_CylinderTest, BoundaryCircleProvidesInteriorMaximum)
{
  const gp_Cylinder  aCylinder(gp_Ax3(gp_Pnt(0.0, 0.0, 0.0), gp_Dir(0.0, 0.0, 1.0)), 10.0);
  ExtremaPS_Cylinder anEvaluator(aCylinder, ExtremaPS::Domain2D(0.0, MathUtils::THE_2PI, 0.0, 1.0));

  const ExtremaPS::Result& aResult =
    anEvaluator.PerformWithBoundary(gp_Pnt(20.0, 0.0, 5.0), THE_TOL, ExtremaPS::SearchMode::Max);

  ASSERT_TRUE(aResult.IsDone());
  ASSERT_NE(aResult.MaxIndex(), ExtremaPS::Result::INVALID_INDEX);
  const ExtremaPS::ExtremumResult& aMaximum = aResult[aResult.MaxIndex()];
  EXPECT_FALSE(aMaximum.IsMinimum);
  EXPECT_NEAR(aMaximum.U, MathUtils::THE_PI, THE_TOL);
  EXPECT_NEAR(aMaximum.V, 0.0, THE_TOL);
  EXPECT_NEAR(aMaximum.SquareDistance, 925.0, THE_TOL);
}
