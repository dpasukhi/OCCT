...
TEST_F(ExtremaPS_SurfaceOfExtrusionTest, DiagonalExtrusionSatisfiesExactReduction)
{
  const gp_Pnt                 aPoint(8.0, -1.0, 3.0);
  ExtremaPS_SurfaceOfExtrusion anExtPS(myDiagonalExtrusion,
                                       ExtremaPS::Domain2D(0.0, 2.0 * M_PI, -20.0, 20.0));
  ...
  const gp_XYZ& aDirection = myDiagonalExtrusion->Direction().XYZ();
  for (size_t anIndex = 0; anIndex < aResult.NbExt(); ++anIndex)
  {
    const ExtremaPS::ExtremumResult& anExtremum = aResult[anIndex];
    const gp_Pnt aBasisPoint = myDiagonalExtrusion->BasisCurve()->EvalD0(anExtremum.U);
    const double anExpectedV = (aPoint.XYZ() - aBasisPoint.XYZ()).Dot(aDirection);
    EXPECT_NEAR(anExtremum.V, anExpectedV, 1.0e-9);

    const Geom_Surface::ResD1 aD1 = myDiagonalExtrusion->EvalD1(anExtremum.U, anExtremum.V);
    const gp_Vec              aResidual(aPoint, aD1.Point);
    EXPECT_NEAR(aResidual.Dot(aD1.D1U), 0.0, 1.0e-5);
...
TEST_F(ExtremaPS_SurfaceOfExtrusionTest, NaturalLineExtrusionUsesExactUnboundedSolution)
{
  ExtremaPS_SurfaceOfExtrusion anExtPS(myPlaneLike);

  const gp_Pnt             aPoint(3.0, 4.0, 7.0);
  const ExtremaPS::Result& aResult =
    anExtPS.Perform(aPoint, THE_TOLERANCE, ExtremaPS::SearchMode::Min);

  ASSERT_EQ(aResult.Status, ExtremaPS::Status::OK);
  ASSERT_EQ(aResult.NbExt(), 1);
  EXPECT_NEAR(aResult[0].U, 3.0, THE_DIST_TOLERANCE);
  EXPECT_NEAR(aResult[0].V, 7.0, THE_DIST_TOLERANCE);
  EXPECT_NEAR(aResult[0].Point.Distance(gp_Pnt(3.0, 0.0, 7.0)), 0.0, THE_DIST_TOLERANCE);
  EXPECT_NEAR(aResult[0].SquareDistance, 16.0, THE_DIST_TOLERANCE);
}
...
TEST_F(ExtremaPS_SurfaceOfExtrusionTest, InteriorMaximumIsSaddle)
{
  ExtremaPS_SurfaceOfExtrusion anExtPS(myCylinderLike,
                                       ExtremaPS::Domain2D(0.0, 2.0 * M_PI, -5.0, 5.0));

  const ExtremaPS::Result& anInteriorResult =
    anExtPS.Perform(gp_Pnt(10.0, 0.0, 0.0), THE_TOLERANCE, ExtremaPS::SearchMode::Max);
  EXPECT_EQ(anInteriorResult.Status, ExtremaPS::Status::NoSolution);
  EXPECT_EQ(anInteriorResult.NbExt(), 0);

  const ExtremaPS::Result& aBoundaryResult =
    anExtPS.PerformWithBoundary(gp_Pnt(10.0, 0.0, 0.0), THE_TOLERANCE, ExtremaPS::SearchMode::Max);
  ASSERT_EQ(aBoundaryResult.Status, ExtremaPS::Status::OK);
  ASSERT_NE(aBoundaryResult.MaxIndex(), ExtremaPS::Result::INVALID_INDEX);
  const ExtremaPS::ExtremumResult& aMaximum = aBoundaryResult[aBoundaryResult.MaxIndex()];
  EXPECT_NEAR(aMaximum.U, M_PI, THE_TOLERANCE);
  EXPECT_NEAR(std::abs(aMaximum.V), 5.0, THE_TOLERANCE);
  EXPECT_NEAR(aMaximum.SquareDistance, 250.0, THE_DIST_TOLERANCE);
  EXPECT_FALSE(aMaximum.IsMinimum);
}
...
TEST_F(ExtremaPS_SurfaceOfExtrusionTest, EdgeCase_PointOnExtrusionLine)
{
  ...
  size_t aMinIdx    = 0;
  for (size_t i = 1; i < aResult.NbExt(); ++i)
  {
