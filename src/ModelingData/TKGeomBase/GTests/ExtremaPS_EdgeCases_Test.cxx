...
//==================================================================================================
// Cone Edge Cases: stationary ring for a point on the axis
//==================================================================================================

TEST_F(ExtremaPS_EdgeCasesTest, Cone_PointOnAxis_CorrectStationaryDistance)
{
  // Cone with 45-degree semi-angle and apex at the origin
...
{
  // Test constrained stationary ring when the unconstrained V is outside the domain
  const double aSemiAngle = M_PI / 4.0;
  const double aCosAngle  = std::cos(aSemiAngle);
  gp_Cone      aCone(gp_Ax3(gp_Pnt(0, 0, 0), gp_Dir(0, 0, 1)), aSemiAngle, 0.0);

  // Point on axis at Z = 20, but stationary V = 20*cos(45) is outside [0, 10]
  ...
  const double aRadiusAtVMax = 10.0 * std::sin(aSemiAngle);
  const double aDeltaZ       = 20.0 - aZAtVMax;
  EXPECT_NEAR(aResult.InfiniteSquareDistance,
              aRadiusAtVMax * aRadiusAtVMax + aDeltaZ * aDeltaZ,
              THE_TOL);
}
