...
bool findExactPolePlane(const NCollection_Array1<gp_Pnt>& thePoles, gp_Pln& thePlane)
{
  ...
  for (const gp_Pnt& aPole : thePoles)
  {
    ...
    const long double aTriple =
      static_cast<long double>(aBase.X())
                       ...
                       occ::handle<Geom2d_Curve>&     theCurve2d)
{
  ...
  if (!aTrimmed.IsNull())
  {
    ...
    theCurve2d = new Geom2d_TrimmedCurve(aBasis2d,
                                         aTrimmed->FirstParameter(),
                                         aTrimmed->LastParameter());
    return true;
  }
                              ...
                              occ::handle<Geom2d_BezierCurve>&      theCurve2d)
{
  ...
  NCollection_Array1<gp_Pnt2d> aPoles(theCurve->Poles().Lower(), theCurve->Poles().Upper());
  for (int anIndex = aPoles.Lower(); anIndex <= aPoles.Upper(); ++anIndex)
  {
    aPoles(anIndex) = ProjLib::Project(thePlane, theCurve->Poles().Value(anIndex));
  }
  theCurve2d = theCurve->IsRational()
                 ? new Geom2d_BezierCurve(aPoles, theCurve->WeightsArray())
