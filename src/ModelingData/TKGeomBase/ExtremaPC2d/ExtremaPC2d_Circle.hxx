...
public:
    ...
    ExtremaPC2d::SearchMode theMode = ExtremaPC2d::SearchMode::MinMax) const
  {
    ...
    int aStart = (theMode == ExtremaPC2d::SearchMode::Max) ? 1 : 0;
    int aEnd   = (theMode == ExtremaPC2d::SearchMode::Min) ? 1 : 2;
    ...
    {
      for (double aU = aSolutions[i];; aU += 2.0 * M_PI)
      {
        if (myDomain.has_value())
        {
          const bool isPastUpper = isClosedDomain
                                     ? aU >= myDomain->Max - aTolU
                                     : aU > myDomain->Max + aTolU;
          if (isPastUpper)
          {
            break;
          }
          aU = std::max(aU, myDomain->Min);
        }
