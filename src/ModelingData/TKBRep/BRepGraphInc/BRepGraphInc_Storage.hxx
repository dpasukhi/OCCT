...
public:
  ...
  void ForEachShapeBinding(FuncT&& theFunc) const
  {
    ...
    for (NCollection_PagedDataMap<TopoDS_Shape, BRepGraph_NodeId, TopTools_ShapeMapHasher>::Iterator
  ...
  {
    std::shared_lock<std::shared_mutex> aLock(myShapeBindingsMutex);
    for (NCollection_PagedDataMap<BRepGraph_NodeId, TopoDS_Shape>::Iterator anIt(myOriginalShapes);
         anIt.More();
         anIt.Next())
    {
      theFunc(anIt.Key(), anIt.Value());
