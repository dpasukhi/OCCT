...
public:
  ...
  {
    const uint64_t aSeed = opencascade::MurmurHash::optimalSeed<uint64_t>();
    for (size_t anIndex = 0; anIndex < myWords.size(); ++anIndex)
    {
      const uint8_t aStreamId = static_cast<uint8_t>(anIndex);
      myWords[anIndex] = opencascade::MurmurHash::hash_combine(aStreamId, sizeof(aStreamId), aSeed);
    }
  ...
  void AppendBytes(const void* theData, size_t theSize)
  {
    ...
    if (myBufferSize != 0)
    {
      const size_t aCopySize = std::min(theSize, myBuffer.size() - myBufferSize);
      std::memcpy(myBuffer.data() + myBufferSize, aBytes, aCopySize);
      myBufferSize += aCopySize;
      aBytes += aCopySize;
      theSize -= aCopySize;
      if (myBufferSize == myBuffer.size())
      {
        hashBlock(myBuffer.data(), myBuffer.size());
        myBufferSize = 0;
      }
    }

    while (theSize >= myBuffer.size())
    {
      hashBlock(aBytes, myBuffer.size());
      aBytes += myBuffer.size();
      theSize -= myBuffer.size();
    }
  ...
  [[nodiscard]] BRepGraph_RevisionHash Finish()
  {
    if (myBufferSize != 0)
    {
      hashBlock(myBuffer.data(), myBufferSize);
      myBufferSize = 0;
    }
    BRepGraph_RevisionHash aResult;
    for (size_t anIndex = 0; anIndex < myWords.size(); ++anIndex)
    {
      aResult.Words[anIndex] = myWords[anIndex];
    }
    return aResult;
  }

private:
  void hashBlock(const void* theData, const size_t theSize)
  {
    for (uint64_t& aWord : myWords)
    {
      aWord = opencascade::MurmurHash::MurmurHash64A(theData, static_cast<int>(theSize), aWord);
    }
  }

  std::array<uint64_t, 4>       myWords;
  std::array<uint8_t, 4 * 1024> myBuffer;
                                        ...
                                        const BRepGraph_RevisionHash& theSemanticHash)
{
  ...
  if (theSemanticHash.IsNull() && aDefinition != nullptr)
  {
    ...
    aBuilder.AppendHash(aDefinitionBuilder.Finish());
  }
                                             ...
                                             const BRepGraph_RevisionHash& theSemanticHash)
{
  ...
  if (theSemanticHash.IsNull())
  {
    ...
    aBuilder.AppendHash(aReferenceBuilder.Finish());
  }
  return aBuilder.Finish();
}
...
bool BRepGraph_RevisionHash::Hasher::ByteAccumulator::Finish(BRepGraph_RevisionHash& theHash)
{
  if (!myData || myData->Remaining != 0)
  {
    return false;
  }
  theHash = myData->Builder.Finish();
  myData.reset();
  return true;
}
