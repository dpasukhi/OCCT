...
struct hash<BRepGraphSupInc_ItemUID>
{
  ...
  {
    const size_t aStoreHash = std::hash<uint32_t>{}(theUID.StoreId);
    const size_t aKindHash = std::hash<uint32_t>{}(theUID.Kind);
    const size_t aCountHash = std::hash<uint32_t>{}(theUID.Counter);
    return aStoreHash ^ (aKindHash << 1) ^ (aCountHash << 7);
  }
