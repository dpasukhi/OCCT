...
struct hash<BRepGraphSupInc_Endpoint>
{
  ...
  {
    const size_t aDomainHash = std::hash<uint8_t>{}(static_cast<uint8_t>(theEndpoint.ItemDomain()));
    switch (theEndpoint.ItemDomain())
    {
      case BRepGraphSupInc_Endpoint::Domain::CoreItem:
      {
        ...
        return aDomainHash ^ (std::hash<BRepGraph_ItemId>{}(*aCoreItem) << 1);
      }
      case BRepGraphSupInc_Endpoint::Domain::SupplementalItem:
      {
        ...
        return aDomainHash ^ (std::hash<BRepGraphSupInc_ItemUID>{}(*aSupplementalItem) << 1);
      }
