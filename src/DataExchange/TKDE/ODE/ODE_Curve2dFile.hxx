...
#ifndef _ODE_Curve2dFile_HeaderFile
#define _ODE_Curve2dFile_HeaderFile
...
//! Manages serialization and deserialization of Geom2d_Curve objects
//! to Cap'n Proto format with deduplication support.
class ODE_Curve2dFile : public Standard_Transient
{
  DEFINE_STANDARD_RTTIEXT(ODE_Curve2dFile, Standard_Transient)

public:
  //! Constructor
  Standard_EXPORT ODE_Curve2dFile();

  //! Adds a 2D curve to the file with deduplication.
  ...
  Standard_EXPORT ODE_ObjectRef AddCurve(const Handle(Geom2d_Curve)& theCurve);

  //! Gets a curve by its object reference.
  //! @param theRef Object reference
  //! @return Curve handle (may be null if reference is invalid)
  Standard_EXPORT Handle(Geom2d_Curve) GetCurve(const ODE_ObjectRef& theRef) const;
...
private:
  ...
  struct CurveEntry
  {
    Handle(Geom2d_Curve) MyCurve;     //!< The actual curve object
...
#endif // _ODE_Curve2dFile_HeaderFile
