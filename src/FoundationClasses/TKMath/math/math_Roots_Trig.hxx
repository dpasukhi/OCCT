...
#ifndef _math_Roots_Trig_HeaderFile
#define _math_Roots_Trig_HeaderFile
                                ...
                                double theEps      = 1.5e-12)
{
  ...
  if (std::abs(theA) <= theEps && std::abs(theB) <= theEps)
  {
    ...
    else
    {
      ...
      PolyResult aPoly = Poly::Quadratic(aCC, aBB, aAA);
      if (!aPoly.IsDone())
      {
        aResult.Status = aPoly.Status;
        return aResult;
      }
      if (aPoly.Status == Status::InfiniteSolutions)
      {
        aResult.InfiniteRoots = true;
        return aResult;
      }
  ...
  else
  {
    ...
    PolyResult aPoly = Poly::Quartic(ko0, ko1, ko2, ko3, ko4);
    if (!aPoly.IsDone())
    {
      if (aPoly.Status == Status::InfiniteSolutions)
      {
...
#endif // _math_Roots_Trig_HeaderFile
