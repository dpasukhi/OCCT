                        ...
                        const Config& theConfig = Config())
{
  ...
  for (int anIter = 0; anIter < theConfig.MaxIterations; ++anIter)
  {
    ...
    if (std::abs(aFa - aFc) > Internal::THE_ZERO_TOL
        && std::abs(aFb - aFc) > Internal::THE_ZERO_TOL)
    {
    ...
    const double aTol = 2.0 * Internal::THE_EPSILON * std::abs(aB) + 0.5 * theConfig.XTolerance;
