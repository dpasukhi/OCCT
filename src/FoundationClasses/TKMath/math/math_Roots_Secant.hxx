...
#ifndef _math_Roots_Secant_HeaderFile
#define _math_Roots_Secant_HeaderFile
...
ScalarResult Secant(Function&     theFunc,
                    double        theX0,
                    double        theX1,
                    const Config& theConfig = Config())
{
  ...
  for (int anIter = 0; anIter < theConfig.MaxIterations; ++anIter)
  {
    ...
    // Check convergence
    if (Internal::IsFConverged(aF1, theConfig.FTolerance))
    {
    ...
    // Check X convergence
    if (Internal::IsXConverged(aX1, aXNew, theConfig.XTolerance))
    {
      double aFNew = 0.0;
...
//! @return result containing root location and convergence status
template <typename Function>
ScalarResult SecantAuto(Function&     theFunc,
                        double        theX0,
                        const Config& theConfig = Config())
{
...
#endif // _math_Roots_Secant_HeaderFile
