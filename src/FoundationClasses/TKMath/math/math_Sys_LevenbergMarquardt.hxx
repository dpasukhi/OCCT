...
#ifndef _math_Sys_LevenbergMarquardt_HeaderFile
#define _math_Sys_LevenbergMarquardt_HeaderFile
                                ...
                                const LMConfig&    theConfig = LMConfig())
{
  ...
  for (int anIter = 0; anIter < theConfig.MaxIterations; ++anIter)
  {
    ...
    for (int aLamIter = 0; aLamIter < 20 && !aStepAccepted; ++aLamIter)
    {
      ...
      auto aLinResult = Lin::Solve(aDamped, aNegJtF);
                                       ...
                                       const LMConfig&    theConfig = LMConfig())
{
  ...
  for (int i = aVarLower; i <= aVarUpper; ++i)
  {
    aSol(i) = Internal::Clamp(aSol(i), theInfBound(i), theSupBound(i));
  }
  ...
  for (int anIter = 0; anIter < theConfig.MaxIterations; ++anIter)
  {
    ...
    for (int aLamIter = 0; aLamIter < 20 && !aStepAccepted; ++aLamIter)
    {
      ...
      for (int i = aVarLower; i <= aVarUpper; ++i)
      {
        aSolNew(i) = Internal::Clamp(aSol(i) + aDeltaX(i), theInfBound(i), theSupBound(i));
      }
...
#endif // _math_Sys_LevenbergMarquardt_HeaderFile
