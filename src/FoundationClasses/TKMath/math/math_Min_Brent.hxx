...
#include <math_InternalBracket.hxx>

#include <cmath>

//! Minimization algorithms for scalar and vector functions.
namespace math
{
                        ...
                        const Config& theConfig = Config())
{
  ...
  double aX = aA + Internal::THE_GOLDEN_SECTION * (aB - aA);
  double aW = aX;
  double aV = aX;
  ...
  for (int anIter = 0; anIter < theConfig.MaxIterations; ++anIter)
  {
    ...
    const double aTol1 =
      theConfig.XTolerance * std::abs(aX) + Internal::THE_ZERO_TOL / 10.0;
    ...
    if (std::abs(aE) > aTol1)
    {
          ...
          && aP < aQ * (aB - aX))
      {
        ...
        {
          aD = Internal::SignTransfer(aTol1, aXm - aX);
        }
        aUseParabolic = true;
      }
    }
    ...
    if (!aUseParabolic)
    {
      ...
      aD = Internal::THE_GOLDEN_SECTION * aE;
    }
    ...
    {
      aU = aX + Internal::SignTransfer(aTol1, aD);
    }
                                   ...
                                   const Config& theConfig = Config())
{
  ...
  Internal::MinBracketResult aBracket =
    Internal::BracketMinimum(theFunc, theGuess, theGuess + theStep);
