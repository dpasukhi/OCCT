...
#ifndef _math_Lin_Householder_HeaderFile
#define _math_Lin_Householder_HeaderFile
...
inline QRResult QR(const math_Matrix& theA, double theTolerance = 1.0e-20)
{
  ...
  for (int j = aColLower; j <= aColUpper; ++j)
  {
    ...
    for (int i = aRowLower + aJOffset; i <= aRowUpper; ++i)
    {
      aVNormSq += Internal::Sqr(aV(i));
    }

    if (aVNormSq < Internal::THE_ZERO_TOL)
    {
      continue;
    }
...
#endif // _math_Lin_Householder_HeaderFile
