...
#ifndef _math_Min_Newton_HeaderFile
#define _math_Min_Newton_HeaderFile
                    ...
                    const NewtonConfig& theConfig = NewtonConfig())
{
  ...
  {
    aGradNorm += Internal::Sqr(aGrad(i));
  }
  ...
  for (int anIter = 0; anIter < theConfig.MaxIterations; ++anIter)
  {
    ...
    // Try to solve H * p = -g
    auto aLinResult = Lin::Solve(aHessian, aNegGrad);
    ...
    if (!aLinResult.IsDone())
    {
      ...
      for (int k = 0; k < 10 && !aSolved; ++k)
      {
        ...
        aLinResult = Lin::Solve(aRegHessian, aNegGrad);
        if (aLinResult.IsDone())
        {
          aSolved = true;
        }
  ...
  perform_line_search:
    ...
    {
      // Line search
      Internal::LineSearchResult aLineResult =
          Internal::ArmijoBacktrack(theFunc, aX, aDir, aGrad, aFx, 1.0, 1.0e-4, 0.5, 50);
      ...
      if (!aLineResult.IsValid)
      {
        ...
        aLineResult = Internal::ArmijoBacktrack(theFunc, aX, aDir, aGrad, aFx, 1.0, 1.0e-4, 0.5, 50);
    ...
    for (int i = aLower; i <= aUpper; ++i)
    {
      aGradNorm += Internal::Sqr(aGradNew(i));
    }
    aGradNorm = std::sqrt(aGradNorm);
                             ...
                             const NewtonConfig& theConfig   = NewtonConfig())
{
  ...
  public:
    ...
    {
      math_Vector aXMod = theX;
      return Internal::NumericalGradientAdaptive(myFunc, aXMod, theGrad, myGradStep);
    }
    ...
    bool Hessian(const math_Vector& theX, math_Matrix& theHess)
    {
      ...
      for (int j = aLower; j <= aUpper; ++j)
      {
        ...
        if (!Internal::NumericalGradientAdaptive(myFunc, aXMod, aGradPlus, myGradStep))
        {
          aXMod(j) = aXj;
          return false;
        }
        ...
        aXMod(j) = aXj - myHessStep;
        if (!Internal::NumericalGradientAdaptive(myFunc, aXMod, aGradMinus, myGradStep))
        {
          aXMod(j) = aXj;
          return false;
        }
...
#endif // _math_Min_Newton_HeaderFile
