...
#ifndef _math_Min_FRPR_HeaderFile
#define _math_Min_FRPR_HeaderFile
                  ...
                  const FRPRConfig&  theConfig = FRPRConfig())
{
  ...
  {
    aGradNormSq += Internal::Sqr(aGrad(i));
  }
  ...
  for (int anIter = 0; anIter < theConfig.MaxIterations; ++anIter)
  {
    ...
    for (int i = aLower; i <= aUpper; ++i)
    {
      aGradNewNormSq += Internal::Sqr(aGradNew(i));
    }
    ...
    else
    {
      switch (theConfig.Formula)
      {
        ...
        case ConjugateGradientFormula::HestenesStiefel:
        {
          ...
          if (std::abs(aDen) > Internal::THE_ZERO_TOL)
          {
            aBeta = aNum / aDen;
          }
          if (aBeta < 0.0)
...
#endif // _math_Min_FRPR_HeaderFile
