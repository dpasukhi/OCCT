...
inline PolyResult Linear(double theA, double theB)
{
  ...
  {
    if (Internal::IsZero(theB))
    {
      aResult.Status = Status::InfiniteSolutions;
    }
    else
    {
      aResult.Status = Status::NoSolution;
    }
    return aResult;
  }
...
inline PolyResult Quadratic(double theA, double theB, double theC)
{
  ...
  const double aDiscTol = Internal::THE_ZERO_TOL * (aB * aB + std::abs(4.0 * aA * aC));
  ...
  const double aQ = -0.5 * (aB + Internal::SignTransfer(aSqrtDisc, aB));
