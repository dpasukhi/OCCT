...
#ifndef _math_Poly_Quartic_HeaderFile
#define _math_Poly_Quartic_HeaderFile
...
inline PolyResult Quartic(double theA, double theB, double theC, double theD, double theE)
{
  ...
  if (Internal::IsZero(q))
  {
    // Substitute u = t^2 to get u^2 + pu + r = 0
    PolyResult aQuadResult = Quadratic(1.0, p, r);
    ...
    Internal::SortRoots(aResult.Roots.data(), aResult.NbRoots);
    aResult.NbRoots = Internal::RemoveDuplicateRoots(aResult.Roots.data(), aResult.NbRoots);

    return aResult;
  }

  // Ferrari's resolvent cubic: y^3 + (p/2)y^2 + ((p^2 - 4r)/16)y - q^2/64 = 0
  // Multiply by 8: 8y^3 + 4py^2 + (p^2 - 4r)/2 * y - q^2/8 = 0
  // Or use: y^3 - (p/2)y^2 - ry + (4pr - q^2)/8 = 0 (different form)

  // Standard Ferrari resolvent: m^3 + (p/2)m^2 + ((p^2-4r)/16)m - q^2/64 = 0
  // Simplified: 8m^3 + 4pm^2 + (p^2 - 4r)m/2 - q^2/8 = 0
  // Or: m^3 + (p/2)m^2 + ((p^2-4r)/16)m - q^2/64 = 0

  // Alternative resolvent (more stable): solve for y where m = 2y
  // y^3 + py^2/2 + (p^2 - 4r)y/16 - q^2/64 = 0
  // Multiply by 64: 64y^3 + 32py^2 + 4(p^2-4r)y - q^2 = 0

  // Use resolvent: z^3 + 2pz^2 + (p^2 - 4r)z - q^2 = 0
  PolyResult aCubicResult = Cubic(1.0, 2.0 * p, p * p - 4.0 * r, -q * q);
  ...
  if (Internal::IsZero(s))
  {
    // Special case: s = 0
    // t^4 + pt^2 + r = (t^2 + u)(t^2 + v) where u + v = p, uv = r
    PolyResult aUVResult = Quadratic(1.0, p, r);
    ...
    if (u <= Internal::THE_ZERO_TOL)
    {
      ...
      if (aSqrt > Internal::THE_ZERO_TOL)
      {
        aResult.Roots[aResult.NbRoots++] = -aSqrt - aShift;
      }
    }

    if (v <= Internal::THE_ZERO_TOL)
    {
  ...
  else
  {
    ...
    // Solve t^2 + st + u = 0
    PolyResult aQuad1 = Quadratic(1.0, s, u);

    // Solve t^2 - st + v = 0
    PolyResult aQuad2 = Quadratic(1.0, -s, v);
...
#endif // _math_Poly_Quartic_HeaderFile
