...
#ifndef _math_Min_BFGS_HeaderFile
#define _math_Min_BFGS_HeaderFile
...
VectorResult BFGS(Function& theFunc, const math_Vector& theStartingPoint, const Config& theConfig = Config())
{
  ...
  for (int anIter = 0; anIter < theConfig.MaxIterations; ++anIter)
  {
    ...
    if (!aLineResult.IsValid || aLineResult.Alpha < Internal::THE_EPSILON)
    {
                           ...
                           const Config&      theConfig   = Config())
{
  ...
  public:
    ...
    bool Gradient(const math_Vector& theX, math_Vector& theGrad)
    {
      ...
      return Internal::NumericalGradientAdaptive(myFunc, aXMod, theGrad, myStep);
    }
                   ...
                   const Config&      theConfig     = Config())
{
  ...
  for (int anIter = 0; anIter < theConfig.MaxIterations; ++anIter)
  {
    ...
    for (int k = aCount - 1; k >= 0; --k)
    {
      ...
      aAlphaVec(aIdx) =
        aRhoVec(aIdx) * Internal::DotProduct(aSVec.Value(aIdx), aQ);
      for (int i = 1; i <= aN; ++i)
      {
    ...
    {
      const int    aIdx  = (aHead + k) % aM;
      const double aBeta = aRhoVec(aIdx) * Internal::DotProduct(aYVec.Value(aIdx), aR);
      for (int i = 1; i <= aN; ++i)
      {
...
#endif // _math_Min_BFGS_HeaderFile
