...
#ifndef _math_Min_Powell_HeaderFile
#define _math_Min_Powell_HeaderFile
  ...
  for (int anIter = 0; anIter < theConfig.MaxIterations; ++anIter)
  {
    ...
    for (int i = 1; i <= aN; ++i)
    {
      ...
      Internal::LineSearchResult aLineResult =
        Internal::ExactLineSearch(theFunc, aX, aDir, 10.0, theConfig.XTolerance);
    ...
    // Check convergence
    if (2.0 * std::abs(aFp - aFx) <= theConfig.FTolerance * (std::abs(aFp) + std::abs(aFx) + Internal::THE_ZERO_TOL))
    {
    ...
    if (aFptt < aFp)
    {
      const double aT = 2.0 * (aFp - 2.0 * aFx + aFptt) * Internal::Sqr(aFp - aFx - aDel)
                        - aDel * Internal::Sqr(aFp - aFptt);

      if (aT < 0.0)
      {
        // Minimize along new direction
        Internal::LineSearchResult aLineResult =
          Internal::ExactLineSearch(theFunc, aX, aXit, 10.0, theConfig.XTolerance);
                                  ...
                                  const Config&      theConfig = Config())
{
  ...
  for (int anIter = 0; anIter < theConfig.MaxIterations; ++anIter)
  {
    ...
    if (2.0 * std::abs(aFp - aFx)
        <= theConfig.FTolerance * (std::abs(aFp) + std::abs(aFx) + Internal::THE_ZERO_TOL))
    {
      aResult.Status = Status::OK;
      aResult.Solution = aX;
      aResult.Value = aFx;
...
#endif // _math_Min_Powell_HeaderFile
