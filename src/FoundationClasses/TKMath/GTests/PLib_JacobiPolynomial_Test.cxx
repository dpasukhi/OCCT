...
TEST_F(PLibJacobiPolynomialTest, CanonicalMaxValuesData)
{
  ...
  {
    const double* aValues = PLib_JacobiPolynomial::MaxValuesData(anExpected.ConstraintOrder);
    ASSERT_NE(aValues, nullptr);
    EXPECT_EQ(PLib_JacobiPolynomial::MaxValuesSize(anExpected.ConstraintOrder), anExpected.Size);
    EXPECT_DOUBLE_EQ(aValues[0], anExpected.First);
    EXPECT_DOUBLE_EQ(aValues[anExpected.Size - 1], anExpected.Last);
  }

  EXPECT_THROW(PLib_JacobiPolynomial::MaxValuesData(GeomAbs_C3), Standard_ConstructionError);
  EXPECT_THROW(PLib_JacobiPolynomial::MaxValuesSize(GeomAbs_C3), Standard_ConstructionError);
}
