...
namespace
{
  ...
  std::vector<double> GetNewRoots(const math::PolyResult& theResult)
  {
    std::vector<double> aRoots;
    if (theResult.IsDone())
    {
...
TEST(math_Poly_ComparisonTest, Quadratic_TwoDistinctRoots)
{
  // x^2 - 5x + 6 = 0 -> roots at 2 and 3
  const double aA = 1.0, aB = -5.0, aC = 6.0;

  math_DirectPolynomialRoots anOldSolver(aA, aB, aC);
  math::PolyResult            aNewResult = math::Poly::Quadratic(aA, aB, aC);
...
TEST(math_Poly_ComparisonTest, Quadratic_DoubleRoot)
{
...
TEST(math_Poly_ComparisonTest, Quadratic_NoRealRoots)
{
...
TEST(math_Poly_ComparisonTest, Quadratic_NegativeRoots)
{
  // x^2 + 5x + 6 = 0 -> roots at -2 and -3
  const double aA = 1.0, aB = 5.0, aC = 6.0;
...
TEST(math_Poly_ComparisonTest, Quadratic_LargeCoefficients)
{
...
TEST(math_Poly_ComparisonTest, Cubic_ThreeDistinctRoots)
{
  ...
  math::PolyResult            aNewResult = math::Poly::Cubic(aA, aB, aC, aD);
...
TEST(math_Poly_ComparisonTest, Cubic_OneRealRoot)
{
...
TEST(math_Poly_ComparisonTest, Cubic_TripleRoot)
{
...
TEST(math_Poly_ComparisonTest, Cubic_NegativeLeadingCoeff)
{
  // -x^3 + 6x^2 - 11x + 6 = 0 -> roots at 1, 2, 3
  const double aA = -1.0, aB = 6.0, aC = -11.0, aD = 6.0;
...
TEST(math_Poly_ComparisonTest, Quartic_FourDistinctRoots)
{
  // (x-1)(x-2)(x-3)(x-4) = x^4 - 10x^3 + 35x^2 - 50x + 24
  const double aA = 1.0, aB = -10.0, aC = 35.0, aD = -50.0, aE = 24.0;

  math_DirectPolynomialRoots anOldSolver(aA, aB, aC, aD, aE);
  math::PolyResult            aNewResult = math::Poly::Quartic(aA, aB, aC, aD, aE);
...
TEST(math_Poly_ComparisonTest, Quartic_TwoRealRoots)
{
...
TEST(math_Poly_ComparisonTest, Quartic_Biquadratic)
{
...
TEST(math_Poly_ComparisonTest, Quartic_NoRealRoots)
{
...
TEST(math_Poly_ComparisonTest, Quartic_QuadrupleRoot)
{
...
TEST(math_Poly_ComparisonTest, Quadratic_SmallDiscriminant)
{
...
TEST(math_Poly_ComparisonTest, Cubic_SmallCoefficients)
{
...
TEST(math_Poly_ComparisonTest, Quadratic_NumericallyChallengingCase)
{
...
TEST(math_Poly_ComparisonTest, Quartic_SymmetricRoots)
{
