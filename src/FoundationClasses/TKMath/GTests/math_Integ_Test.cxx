...
TEST(math_Integ_GaussTest, ConstantFunction)
{
  ConstantFunc aFunc;
...
TEST(math_Integ_GaussTest, Order3)
{
  QuadraticFunc aFunc;
  math::IntegResult aResult = math::Integ::Gauss(aFunc, 0.0, 1.0, 3);
  ASSERT_TRUE(aResult.IsDone());
...
TEST(math_Integ_GaussTest, InvalidOrder)
{
  ...
  math::IntegResult aResult = math::Integ::Gauss(aFunc, 0.0, THE_PI, 10);
...
TEST(math_Integ_GaussTest, PolynomialExactness_Order3)
{
...
TEST(math_Integ_GaussAdaptiveTest, SineFunction)
{
  SinFunc aFunc;
  math::IntegConfig aConfig;
...
TEST(math_Integ_GaussCompositeTest, SineFunction)
{
  SinFunc aFunc;
...
TEST(math_Integ_GaussCompositeTest, OscillatoryFunction)
{
  OscillatoryFunc aFunc;
...
{
  SinFunc aFunc;
  math::IntegResult aResult = math::Integ::GaussComposite(aFunc, 0.0, THE_PI, 0, 7);
  EXPECT_EQ(aResult.Status, math::Status::InvalidInput);
}
...
TEST(math_Integ_BoundsTest, ReversedBounds)
{
  SinFunc aFunc;
...
TEST(math_Integ_SpecialTest, SqrtFunction)
{
  SqrtFunc aFunc;
  ...
  math::IntegResult aResult = math::Integ::GaussComposite(aFunc, 0.001, 1.0, 20, 15);
  ASSERT_TRUE(aResult.IsDone());
...
TEST(math_Integ_BoolConversionTest, SuccessfulResultIsTrue)
{
  SinFunc aFunc;
  math::IntegResult aResult = math::Integ::Gauss(aFunc, 0.0, THE_PI, 15);
...
TEST(math_Integ_BoolConversionTest, InvalidInputIsFalse)
{
  SinFunc aFunc;
  math::IntegResult aResult = math::Integ::Gauss(aFunc, 0.0, THE_PI, 10);
...
TEST(math_Integ_AccuracyTest, HigherOrderMoreAccurate)
{
  SinFunc aFunc;
  const double aExact = 2.0;

  math::IntegResult aResult3 = math::Integ::Gauss(aFunc, 0.0, THE_PI, 3);
  math::IntegResult aResult7 = math::Integ::Gauss(aFunc, 0.0, THE_PI, 7);
  math::IntegResult aResult15 = math::Integ::Gauss(aFunc, 0.0, THE_PI, 15);
