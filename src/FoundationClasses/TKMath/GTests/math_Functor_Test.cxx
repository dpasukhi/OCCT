...
TEST(math_Functor_Scalar, ScalarLambda_Value)
{
...
TEST(math_Functor_Scalar, ScalarLambdaWithDerivative_Values)
{
  auto aFunc = math::Functor::MakeScalarWithDerivative([](double x, double& y, double& dy) {
    y  = x * x - 2.0;
    dy = 2.0 * x;
    return true;
  });
...
TEST(math_Functor_Scalar, Polynomial_Value)
{
  // p(x) = 1 + 2x + 3x^2
  math::Functor::Polynomial aPoly({1.0, 2.0, 3.0});
...
TEST(math_Functor_Scalar, Polynomial_Values)
{
  // p(x) = x^2 - 2, p'(x) = 2x
  math::Functor::Polynomial aPoly({-2.0, 0.0, 1.0});
...
TEST(math_Functor_Scalar, Polynomial_WithRoots)
{
  // Using Brent's method with polynomial
  // p(x) = x^2 - 2, roots at +/-sqrt(2)
  math::Functor::Polynomial aPoly({-2.0, 0.0, 1.0});

  auto aResult = math::Roots::Brent(aPoly, 0.0, 2.0);
  EXPECT_TRUE(aResult.IsDone());
  EXPECT_TRUE(aResult.Root.has_value());
  EXPECT_NEAR(*aResult.Root, std::sqrt(2.0), 1e-9);
}

TEST(math_Functor_Scalar, Polynomial_WithNewton)
{
  // Using Newton's method with polynomial (has derivative)
  // p(x) = x^3 - 2x - 5
  math::Functor::Polynomial aPoly({-5.0, -2.0, 0.0, 1.0});

  auto aResult = math::Roots::Newton(aPoly, 2.0);
  EXPECT_TRUE(aResult.IsDone());
  EXPECT_TRUE(aResult.Root.has_value());
...
TEST(math_Functor_Scalar, Rational_Value)
{
  // f(x) = (x + 1) / (x^2 + 1)
  math::Functor::Rational aRat({1.0, 1.0}, {1.0, 0.0, 1.0});
...
TEST(math_Functor_Scalar, Rational_DivisionByZero)
{
  // f(x) = x / (x - 1), undefined at x = 1
  math::Functor::Rational aRat({0.0, 1.0}, {-1.0, 1.0});

  double aValue = 0.0;
  EXPECT_FALSE(aRat.Value(1.0, aValue));
}

TEST(math_Functor_Scalar, Composite_Value)
{
  // f(g(x)) where f(x) = x^2, g(x) = x + 1
  // Result: (x+1)^2
  math::Functor::Polynomial aOuter({0.0, 0.0, 1.0}); // x^2
  math::Functor::Polynomial aInner({1.0, 1.0});      // x + 1
  auto aComposite = math::Functor::MakeComposite(aOuter, aInner);
...
TEST(math_Functor_Scalar, Sum_Value)
{
  // f(x) + g(x) = (x^2) + (2x + 1) = x^2 + 2x + 1
  math::Functor::Polynomial aF({0.0, 0.0, 1.0}); // x^2
  math::Functor::Polynomial aG({1.0, 2.0});      // 2x + 1
  auto aSum = math::Functor::MakeSum(aF, aG);
...
TEST(math_Functor_Scalar, Difference_Value)
{
  // f(x) - g(x) = (x^2) - (x) = x^2 - x
  math::Functor::Polynomial aF({0.0, 0.0, 1.0}); // x^2
  math::Functor::Polynomial aG({0.0, 1.0});      // x
  auto aDiff = math::Functor::MakeDifference(aF, aG);
...
TEST(math_Functor_Scalar, Product_Value)
{
  // f(x) * g(x) = (x) * (x + 1) = x^2 + x
  math::Functor::Polynomial aF({0.0, 1.0});      // x
  math::Functor::Polynomial aG({1.0, 1.0});      // x + 1
  auto aProd = math::Functor::MakeProduct(aF, aG);
...
TEST(math_Functor_Scalar, Quotient_Value)
{
  // f(x) / g(x) = (x^2) / (x) = x
  math::Functor::Polynomial aF({0.0, 0.0, 1.0}); // x^2
  math::Functor::Polynomial aG({0.0, 1.0});      // x
  auto aQuot = math::Functor::MakeQuotient(aF, aG);
...
TEST(math_Functor_Scalar, Scaled_Value)
{
  // 3 * f(x) = 3 * x^2
  math::Functor::Polynomial aF({0.0, 0.0, 1.0}); // x^2
  auto aScaled = math::Functor::MakeScaled(aF, 3.0);
...
TEST(math_Functor_Scalar, Shifted_Value)
{
  // f(x) + 5 = x^2 + 5
  math::Functor::Polynomial aF({0.0, 0.0, 1.0}); // x^2
  auto aShifted = math::Functor::MakeShifted(aF, 5.0);
...
TEST(math_Functor_Scalar, Negated_Value)
{
  // -f(x) = -x^2
  math::Functor::Polynomial aF({0.0, 0.0, 1.0}); // x^2
  auto aNegated = math::Functor::MakeNegated(aF);
...
TEST(math_Functor_Scalar, Constant_Value)
{
  math::Functor::Constant aConst(42.0);
...
TEST(math_Functor_Scalar, Linear_Value)
{
  // f(x) = 2x + 3
  math::Functor::Linear aLinear(2.0, 3.0);
...
TEST(math_Functor_Scalar, Sine_Value)
{
  math::Functor::Sine aSine(1.0, 1.0, 0.0, 0.0);
...
TEST(math_Functor_Scalar, Sine_Values)
{
  math::Functor::Sine aSine(2.0, 1.0, 0.0, 0.0); // 2*sin(x)
...
TEST(math_Functor_Scalar, Cosine_Value)
{
  math::Functor::Cosine aCosine(1.0, 1.0, 0.0, 0.0);
...
TEST(math_Functor_Scalar, Exponential_Value)
{
  math::Functor::Exponential aExp(1.0, 1.0, 0.0); // exp(x)
...
TEST(math_Functor_Scalar, Power_Value)
{
  math::Functor::Power aPower(2.0, 1.0, 0.0); // x^2
...
TEST(math_Functor_Scalar, Power_NonInteger)
{
  math::Functor::Power aPower(0.5, 1.0, 0.0); // sqrt(x)
...
TEST(math_Functor_Scalar, Gaussian_Value)
{
  math::Functor::Gaussian aGauss(1.0, 0.0, 1.0); // Standard normal
...
TEST(math_Functor_Vector, VectorLambda_Value)
{
  auto aFunc = math::Functor::MakeVector([](const math_Vector& x, double& y) {
...
TEST(math_Functor_Vector, VectorLambdaWithGradient_Values)
{
  auto aFunc = math::Functor::MakeVectorWithGradient(
...
TEST(math_Functor_Vector, Rosenbrock_Value)
{
  math::Functor::Rosenbrock aRosen; // Default a=1, b=100
...
TEST(math_Functor_Vector, Rosenbrock_AwayFromMinimum)
{
  math::Functor::Rosenbrock aRosen;
...
TEST(math_Functor_Vector, Rosenbrock_Gradient)
{
  math::Functor::Rosenbrock aRosen;
...
TEST(math_Functor_Vector, Sphere_Value)
{
  math::Functor::Sphere aSphere;
...
TEST(math_Functor_Vector, Sphere_Gradient)
{
  math::Functor::Sphere aSphere;
...
TEST(math_Functor_Vector, Booth_Value)
{
  math::Functor::Booth aBooth;
...
TEST(math_Functor_Vector, Beale_Value)
{
  math::Functor::Beale aBeale;
...
TEST(math_Functor_Vector, Himmelblau_Value)
{
  math::Functor::Himmelblau aHimmel;
...
TEST(math_Functor_Vector, Rastrigin_Value)
{
  math::Functor::Rastrigin aRast;
...
TEST(math_Functor_Vector, Ackley_Value)
{
  math::Functor::Ackley aAckley;
...
TEST(math_Functor_Vector, QuadraticForm_Value)
{
  ...
  math::Functor::QuadraticForm aQuad(aA, aB, aC);
...
TEST(math_Functor_Vector, QuadraticForm_Gradient)
{
...
TEST(math_Functor_Vector, LinearResidual_Value)
{
  // Solve: [1 0; 0 1] * x = [3; 4]
  ...
  math::Functor::LinearResidual aRes(aA, aB);
...
TEST(math_Functor_Vector, LinearResidual_NonZero)
{
  math_Matrix aA(1, 2, 1, 2, 0.0);
  aA(1, 1) = 1.0;
  aA(2, 2) = 1.0;
...
TEST(math_Functor_Integration, Polynomial_WithBrent)
{
  // Find root of x^3 - x - 2 = 0 (root near 1.52)
  math::Functor::Polynomial aPoly({-2.0, -1.0, 0.0, 1.0});

  auto aResult = math::Roots::Brent(aPoly, 1.0, 2.0);
  EXPECT_TRUE(aResult.IsDone());
  EXPECT_TRUE(aResult.Root.has_value());
...
TEST(math_Functor_Integration, Sine_RootFinding)
{
  // Find root of sin(x) in [2, 4] (root at pi)
  math::Functor::Sine aSine;

  auto aResult = math::Roots::Brent(aSine, 2.0, 4.0);
  EXPECT_TRUE(aResult.IsDone());
  EXPECT_TRUE(aResult.Root.has_value());
  EXPECT_NEAR(*aResult.Root, THE_PI, 1e-10);
}

TEST(math_Functor_Integration, Sphere_WithPowell)
{
  ...
  auto aResult = math::Min::Powell(aSphere, aStart);
  EXPECT_TRUE(aResult.IsDone());
...
TEST(math_Functor_Integration, Booth_WithBFGS)
{
  ...
  auto aResult = math::Min::BFGS(aBooth, aStart);
  EXPECT_TRUE(aResult.IsDone());
  EXPECT_TRUE(aResult.Solution.has_value());
...
TEST(math_Functor_Integration, Rosenbrock_WithBFGS)
{
...
TEST(math_Functor_Integration, Quadratic_WithNewton)
{
  ...
  math::Functor::QuadraticForm aQuad(aA, aB, 10.0);

  math_Vector aStart(1, 2);
  aStart(1) = 0.0;
  aStart(2) = 0.0;

  auto aResult = math::Min::BFGS(aQuad, aStart);
  EXPECT_TRUE(aResult.IsDone());
  EXPECT_TRUE(aResult.Solution.has_value());
  EXPECT_NEAR((*aResult.Solution)(1), 1.0, 1e-5);
