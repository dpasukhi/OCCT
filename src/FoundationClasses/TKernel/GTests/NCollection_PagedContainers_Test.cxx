...
TEST(NCollection_PagedArrayTest, SupportsStandardAndLegacyIteration)
{
  ...
  NCollection_PagedArray<int>::Iterator anIterator(aValues);
  ASSERT_TRUE(anIterator.More());
  anIterator.ChangeValue() = 3;
  aValues.Append(16);
  int aVisited = 0;
  ...
  NCollection_PagedArray<int>::ConstIterator aConstIterator(aConstValues);
  ASSERT_TRUE(aConstIterator.More());
  EXPECT_EQ(aConstIterator.Value(), 3);
}
...
TEST(NCollection_PagedDataMapTest, SupportsStandardLegacyAndItemIteration)
{
  ...
  NCollection_PagedDataMap<int, int>::Iterator anIterator(aMap);
  ASSERT_TRUE(anIterator.More());
  const int aFirstKey = anIterator.Key();
  anIterator.ChangeValue() += 10;
  anIterator.Next();
  anIterator.Reset();
  ...
  EXPECT_THROW((void)anEnd.Key(), Standard_NoSuchObject);
  EXPECT_THROW((void)anEnd.Value(), Standard_NoSuchObject);
  EXPECT_THROW((void)anEnd.ChangeValue(), Standard_NoSuchObject);
}
