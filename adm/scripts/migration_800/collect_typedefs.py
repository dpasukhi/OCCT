...
class TypedefCollector:
    ...
    # Pattern for NCollection container typedef
    ...
    TEMPLATE_CLASS_PATTERN = re.compile(r'^template\s*<[^>]*>\s*class\s+\w+', re.MULTILINE)
    ...
    def has_non_typedef_content(self, content: str) -> Tuple[bool, bool, bool, bool, bool]:
        """
        ...
        has_template_class = bool(self.TEMPLATE_CLASS_PATTERN.search(clean_content))
    ...
    def extract_typedefs(self, content: str, header_path: str) -> List[TypedefInfo]:
        ...
        for match in self.CONTAINER_TYPEDEF_PATTERN.finditer(content):
            ...
            namespace = self.find_namespace_at_position(content, match.start())
        ...
        for match in self.ITERATOR_TYPEDEF_PATTERN.finditer(content):
            collection_type = match.group(1)
            # Normalize whitespace - collapse newlines and multiple spaces to single space
            template_params = ' '.join(match.group(2).split())
            typedef_name = match.group(3)
        ...
        for match in self.ALIAS_TYPEDEF_PATTERN.finditer(content):
    ...
    def analyze_header(self, file_path: Path) -> Optional[HeaderInfo]:
        ...
        has_class, has_function, has_template_class, has_enum, has_other = \
            self.has_non_typedef_content(content)
    ...
    def collect(self) -> Dict:
        ...
        for typedef in resolved_aliases:
            ...
            if header_path in self.headers:
                ...
                # New header with only alias typedef
                header_info = HeaderInfo(
                    relative_path=header_path,
                    is_typedef_only=True,  # Alias typedef headers are typically typedef-only
                    typedefs=[typedef]
                )
